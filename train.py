...
def evaluate_model(model, X_val, y_val) -> Dict:
    ...
    metrics = {
        ...
        "precision_macro":  float(round(precision_score(y_val, y_pred, average="macro", zero_division=0), 4)),
        "recall_macro":     float(round(recall_score(y_val, y_pred, average="macro", zero_division=0), 4)),
    }
    for i, cls in enumerate(CLASS_NAMES):
        mask = y_val == i
        if mask.sum() > 0:
            metrics[f"precision_{cls}"] = float(round(precision_score(y_val==i, y_pred==i, zero_division=0), 4))
            metrics[f"recall_{cls}"]    = float(round(recall_score(y_val==i, y_pred==i, zero_division=0), 4))
            metrics[f"f1_{cls}"]        = float(round(f1_score(y_val==i, y_pred==i, zero_division=0), 4))
...
def run_training(data_path: str = None, combine: bool = False) -> Dict:
    ...
    # ── MLflow run ────────────────────────────────────────────────────────────
    with mlflow.start_run(nested=bool(mlflow.active_run()), run_name=f"train_{source_label.replace('/','_')}") as run:
        ...
        mlflow.set_tags({
            ...
            "git_commit":     os.popen("git rev-parse --short HEAD 2>/dev/null").read().strip() or "N/A",
        })
    ...
    print(f"  View UI:           mlflow ui --backend-store-uri {MLFLOW_CFG['tracking_uri']}")
