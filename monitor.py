...
def compute_ks(ref: pd.Series, prod: pd.Series) -> Dict:
    ...
    return {"statistic": round(float(s),4),
...
def run_feature_drift(ref_df, prod_df, features) -> Dict:
    ...
    for feat in features:
        ...
        status = "DRIFT 🚨" if fire else ("WATCH ⚠️" if psi > DRIFT_CFG["psi"]["watch"] else "OK ✅")
        results[feat] = {"psi": psi, "ks": ks, "jsd": jsd, "status": status, "fire": fire}
    ...
    return {
        "per_feature": results,
        "n_drifted": n_drift, "max_psi": psi_max,
...
def monitor_performance(bundle, prod_df) -> Optional[Dict]:
    ...
    return {
        ...
        "trigger_retraining": trig,
        "_mlflow_metrics": {
            "prod_accuracy": acc, "prod_f1_macro": f1,
...
def generate_evidently_report(ref_df, prod_df, report_path) -> Dict:
    try:
        ...
        report = Report(metrics=[DataDriftPreset(), DataQualityPreset(), DatasetDriftMetric()])
        ...
        return {"dataset_drifted": d.get("dataset_drift",False),
...
def run_monitoring_cycle(production_path=None, report_only=False) -> Dict:
    ...
    if os.path.exists(PATHS["current_model"]):
        ...
        print(f"Model: {PATHS['current_model']}  (trained {bundle.get('trained_at','?')})")
    else:
    ...
    if report_only:
        results["evidently"] = generate_evidently_report(ref_df, prod_df, PATHS["drift_report"])
        return results
    ...
    if len(prod_df) >= min_n:
        fd = run_feature_drift(ref_df, prod_df,
                               CFG["triggers"]["drift_based"]["features_to_monitor"])
        results["feature_drift"] = {k:v for k,v in fd.items() if k != "_mlflow_metrics"}
    ...
    if bundle:
        ...
        results["prediction_drift"] = {k:v for k,v in pd_res.items() if k != "_mlflow_metrics"}
        all_mlflow_metrics.update(pd_res.get("_mlflow_metrics", {}))

        perf = monitor_performance(bundle, prod_df)
        if perf:
            results["performance"] = {k:v for k,v in perf.items() if k != "_mlflow_metrics"}
    ...
    # Aggregate trigger
    triggers = []
    if results.get("feature_drift",{}).get("trigger_retraining"):   triggers.append("feature_drift")
    if results.get("prediction_drift",{}).get("trigger_retraining"): triggers.append("prediction_drift")
    if results.get("performance",{}) and results["performance"].get("trigger_retraining"):
    ...
    with mlflow.start_run(nested=True, run_name=f"monitor_{datetime.now(timezone.utc).strftime('%Y%m%d_%H%M%S')}") as mrun:
